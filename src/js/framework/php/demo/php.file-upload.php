...
if (isset($_POST['upload_check'])) {
	...
	if (isset($_FILES['upload']) && !$_FILES['upload']['error']) {
		...
		if (in_array($_FILES['upload']['type'], $imageKind)) {
			...
			if (move_uploaded_file ($_FILES['upload']['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/js/framework/php/demo/upload/{$_FILES['upload']['name']}")) {

				// 5.업로드된 이미지 파일을 출력
				echo '<p><img src="/js/framework/php/demo/upload/'.$_FILES['upload']['name'].'" /></p>';
...
<form enctype="multipart/form-data" action="php.file-upload.php" method="post">
